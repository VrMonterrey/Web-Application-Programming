<template>
  <v-card>
    ...
    <v-card-subtitle>
      <v-row>
        <v-col>
          <v-text-field v-model="search" @input="retrieveTasks" variant="solo" label="Match name"></v-text-field>
        </v-col>
        <v-col cols="2">
          <div>Limit</div>
          <v-slider density="compact" v-model="limit" min="5" max="100" step="5" thumb-label @update:modelValue="retrieveTasks"></v-slider>
        </v-col>
        <v-col cols="1">
          <v-btn variant="elevated" color="success" @click="add" v-if="checkIfInRole(user, [0, 1])">Add</v-btn>
        </v-col>
        <v-col cols="1">
          <v-btn variant="elevated" color="cyan" @click="finish" v-if="checkIfInRole(user, [0, 1])">Finish</v-btn>
        </v-col>
      </v-row>
    </v-card-subtitle>
        ...
        <tbody>
          <template v-for="(project, index) in projects" :key="`project-${index}`">
            <tr>
              ...
              <td class="text-center"><v-chip :color="project.color">{{ project.shortcut }}</v-chip></td>
              ...
              <td class="text-center">
                <v-btn @click="toggleTasks(project._id)">
                  <v-icon>{{ expandedProjects.includes(project._id) ? 'mdi-chevron-up' : 'mdi-chevron-down' }}</v-icon>
                </v-btn>
              </td>
            </tr>
            <tr v-if="expandedProjects.includes(project._id)" :key="`gantt-${index}`">
              <td colspan="5">
                <v-card flat>
                  ...
                  <v-table dense hover>
                    ...
                    <tbody>
                      <tr v-for="(task, tindex) in tasks.filter(task => task.project_id === project._id)" :key="`task-${tindex}`">
                        ...
                        <td class="text-center" @click="click(task)">{{ task.finishDate ? new Date(task.finishDate).toLocaleDateString() : "Unfinished" }}</td>
  ...
  <v-dialog v-model="editor" width="50%">
    <TaskEditor :id="id" @dataChanged="retrieveTasks" @cancel="cancel" @dataAccessFailed="onDataAccessFailed"/>
  </v-dialog>
  ...
  export default {
    name: "TasksLister",
    components: { TaskEditor, GanttChart },
    mixins: [common],
